...
public void testLogoutFromEditEventPage() {
    ...
    @Test
    public void testNavigationWithoutSavingChanges() {
        // Locate the event name field, clear it, and enter new data
        WebElement eventName = driver.findElement(By.name("eventName"));
        eventName.clear();
        eventName.sendKeys("Temporary Change");

        // Navigate away by clicking on another link (e.g., "My Events")
        driver.findElement(By.xpath("//*[@id=\"root\"]/div/div/nav/ul/li[3]")).click();

        // Return to the edit page to check if the change was discarded
        driver.findElement(By.xpath("//*[@id=\"root\"]/div/div/nav/ul/li[3]")).click(); // Adjust with the correct navigation to return

        // Verify that the changes were not saved (the input field should not contain the "Temporary Change" text)
        eventName = wait.until(ExpectedConditions.visibilityOfElementLocated(By.name("eventName")));
        String currentEventName = eventName.getAttribute("value");
        assertTrue("Unsaved changes were retained after navigation.", !currentEventName.equals("Temporary Change"));
    }
